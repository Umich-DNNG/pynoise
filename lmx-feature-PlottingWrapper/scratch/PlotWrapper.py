...
# input path
#Example: filepath = r'X:\Scratch\Hutchinson\FAUSTlmxModified\FAUST\lmx\scratch\input\SCRAP_bare\split'
filepath = ''
...
input_type = '.xml'
#Example: input_type = '.lmx'
#Example: input_type = '.csv'
# this is a tuple used for plot legends. example below.
file_descriptions = ()
#Example: file_descriptions = ('Config 1','Config 2','Config 3')
...
# Do you want to perform multiple randomized fits?
Y2_fit_randomized_data = False
# This is the number of points to use in the fit. Options are either the string 'automatic' or an integer that must be less than the number of gates. 
Y2_fit_randomized_data_number_points = 'automatic'
# This is the total number of fits that will be performed.
Y2_fit_randomized_data_number_fits = 10000
...
# Plotting options
# Put True if you are plotting 'plot_all_files_vs_con' and you want the x-axis to be the configuration description (instead of an arbitrary file number)
des_on_vs_config = False
...
# channel display options include 'MC15_1' for a single MC-15, 'MC15_2' for two MC-15s, and 'Other', which will just display all non-zero channels
channel_display = ''
# include a title: True or False
use_title = True
title_prefix = 'NOMAD'
...
# Put True if you want to include background subtraction
perform_bkg_subtract_rates = False
# If you want to perform background subtraction, include the filename (extension required to be the same as foreground and the path is the same as filepath)
# File type for bkg must be .xml or .lmx
bkg_subtract_fname = ''
...
# input paths
if perform_bkg_subtract_rates == True:
    files, bkg_file = input_paths_w_bkg(input_type,filepath,bkg_subtract_fname)
else:    
...
if plot_all_files_vs_con == True and des_on_vs_config == True and len(files) != len(file_descriptions):
    sys.exit('If "des_on_vs_config" is set to True then the length of "file_descriptions" must be the same as the number of files present.')
if plot_channels == True:
    if channel_display != 'MC15_1' and channel_display != 'MC15_2' and channel_display != 'Other':
        sys.exit('If "plot_channels" is set to True, then "channel_display" must be a supported type (currently "MC15_1", "MC15_2", or "Other".')
if Y2_fit_randomized_data == True:
    if perform_Y2_single_fit != True and perform_Y2_double_fit != True:
        sys.exit('Exit. "Y2_fit_randomized_data" is set to True but neither "perform_Y2_single_fit" nor "perform_Y2_double_fit" are set to True. At least one of these must be True to perform Y2 fitting.')


if des_on_vs_config == True:
    config_x_label = file_descriptions
else:
    config_x_label = file_number
...
if perform_bkg_subtract_rates == True:
    ...
    if plot_histograms == True or plot_individual_files == True or plot_all_files_vs_gw == True or plot_all_files_vs_con == True or plot_channels == True or perform_Rossi == True or read_header == True:
        ...
        if input_type == '.lmx':
            ...
            bkg_events_list = []
            [bkg_events_list.extend(eventsFromLMX(bkg_file))]
                       
            # splits that list by time and channel
            bkg_time_list = [event.time for event in bkg_events_list]
            bkg_channel_list = [event.detector for event in bkg_events_list]
            
            bkg_time_diff_list = []
            for i in range(1,len(bkg_time_list)):
                bkg_time_diff_list.append(bkg_time_list[i]-bkg_time_list[i-1])
            if np.min(bkg_time_diff_list) > np.max(gatewidth_list):
                print('Min time diff in BKG file = ',np.min(bkg_time_diff_list),' is greater than the maximum gate-width ',np.max(gatewidth_list),' by a ratio of ',np.min(bkg_time_diff_list)/np.max(gatewidth_list),'. It likely is safe to assume that BKG R2 is 0.')
            else:
                print('Min time diff in BKG file = ',np.min(bkg_time_diff_list),' is less than the maximum gate-width ',np.max(gatewidth_list),' by a ratio of only ',np.min(bkg_time_diff_list)/np.max(gatewidth_list),'. It likely is NOT safe to assume that BKG R2 is 0. Check that the file is indeed BKG.')                
            
            
            # this outputs a csv file with all events if set to True by the user
            if output_event_csv == True:        
        
                with open(save_path+bkg_file+'_time_list.csv', mode='w', newline='') as csv_file:
                    writer = csv.writer(csv_file)
                    for current_time in bkg_time_list:
                        writer.writerow([current_time])
                        
                with open(save_path+bkg_file+'_event_list.csv', mode='w', newline='') as csv_file:
                    writer = csv.writer(csv_file)
                    for i in range(0,len(bkg_time_list)):
                        current_time = bkg_time_list[i]
                        current_channel = bkg_channel_list[i]
            ...
            if plot_histograms == True or plot_individual_files == True or plot_all_files_vs_gw == True or plot_all_files_vs_con == True or perform_Y2_single_fit == True or perform_Y2_double_fit == True:
                ...
                bkg_feynman_calculator = FeynmanHistogramCalculator(bkg_events_list, binning=binning)
                print('Feynman Calculator Complete for BKG file')
                print(strftime("%a, %d %b %Y %H:%M:%S +0000", gmtime()))
        
        # create empty lists that include data from a single file only
        bkg_histogram_x_list, bkg_histogram_y_list, bkg_histogram_P_list, bkg_first_reduced_factorial_moment_list, bkg_second_reduced_factorial_moment_list, bkg_third_reduced_factorial_moment_list, bkg_fourth_reduced_factorial_moment_list, bkg_first_factorial_moment_list, bkg_second_factorial_moment_list, bkg_third_factorial_moment_list, bkg_fourth_factorial_moment_list, bkg_single_feynman_mean_list, bkg_single_feynman_variance_list, bkg_single_variance_to_mean_list, bkg_Y1_list, bkg_dY1_list,  bkg_R1_list, bkg_dR1_list, bkg_dR1R2_list, bkg_Y2_list, bkg_dY2_list, bkg_Ym_list, bkg_dYm_list, bkg_histogram_x_sum_list, bkg_histogram_y_sum_list, bkg_histogram_P_sum_list = ([] for i in range(26))
        ...
        if plot_individual_files == True or perform_Y2_single_fit == True or perform_Y2_double_fit == True:
            print('Plotting individual files start for file ',bkg_subtract_fname)
            print(strftime("%a, %d %b %Y %H:%M:%S +0000", gmtime()))
            current_save_path = output_subdirectory_paths(output_nested_subdirectories, save_path, 'Individual_Files/Rates/', '', bkg_subtract_fname, add_current_file=True, make_dirs=True)
        ...
        if plot_histograms == True or plot_individual_files == True or plot_all_files_vs_gw == True or plot_all_files_vs_con == True or perform_Y2_single_fit == True or perform_Y2_double_fit == True:
            ...
            if input_type == '.xml':
                ...
                gatewidth_list, bkg_feynman_data = readXML(bkg_subtract_fname, xml_user_gatewidths, gatewidth_list)
            ...
            for gatewidth in gatewidth_list:
                if input_type == '.lmx':
                    ...
                    bkg_single_feynman_histogram = bkg_feynman_calculator.calculate(gatewidth)
                    bkg_first_reduced_factorial_moment = bkg_single_feynman_histogram.reduced_factorial_moment(1)
                    bkg_first_reduced_factorial_moment_list.append(bkg_first_reduced_factorial_moment)
                    bkg_second_reduced_factorial_moment = bkg_single_feynman_histogram.reduced_factorial_moment(2)
                    bkg_second_reduced_factorial_moment_list.append(bkg_second_reduced_factorial_moment)
                    bkg_third_reduced_factorial_moment = bkg_single_feynman_histogram.reduced_factorial_moment(3)
                    bkg_third_reduced_factorial_moment_list.append(bkg_third_reduced_factorial_moment)
                    bkg_fourth_reduced_factorial_moment = bkg_single_feynman_histogram.reduced_factorial_moment(4)
                    bkg_fourth_reduced_factorial_moment_list.append(bkg_fourth_reduced_factorial_moment)
                    bkg_first_factorial_moment = bkg_single_feynman_histogram.factorial_moment(1)
                    bkg_first_factorial_moment_list.append(bkg_first_factorial_moment)
                    bkg_second_factorial_moment = bkg_single_feynman_histogram.factorial_moment(2)
                    bkg_second_factorial_moment_list.append(bkg_second_factorial_moment)
                    bkg_third_factorial_moment = bkg_single_feynman_histogram.factorial_moment(3)
                    bkg_third_factorial_moment_list.append(bkg_third_factorial_moment)
                    bkg_fourth_factorial_moment = bkg_single_feynman_histogram.factorial_moment(4)   
                    bkg_fourth_factorial_moment_list.append(bkg_fourth_factorial_moment)
                    
                    bkg_single_feynman_mean = bkg_single_feynman_histogram.mean
                    bkg_single_feynman_mean_list.append(bkg_single_feynman_mean)
                    bkg_single_feynman_variance = bkg_single_feynman_histogram.variance
                    bkg_single_feynman_variance_list.append(bkg_single_feynman_variance)
                    bkg_single_variance_to_mean = bkg_single_feynman_histogram.variance_to_mean
                    bkg_single_variance_to_mean_list.append(bkg_single_variance_to_mean)
                
                    bkg_Y1, bkg_dY1 = bkg_single_feynman_histogram.Y1
                    bkg_R1 = bkg_Y1
                    bkg_dR1 = bkg_dY1
                    bkg_Y1_list.append(bkg_Y1)
                    bkg_R1_list.append(bkg_R1)
                    bkg_dY1_list.append(bkg_dY1)
                    bkg_dR1_list.append(bkg_dR1)
                    bkg_Y2, bkg_dY2 = bkg_single_feynman_histogram.Y2
                    bkg_Y2_list.append(bkg_Y2)
                    bkg_dY2_list.append(bkg_dY2)
                    # Equation 18
                    bkg_Ym = ((bkg_second_factorial_moment-bkg_first_factorial_moment**2)/bkg_first_factorial_moment)-1
                    bkg_Ym_list.append(bkg_Ym)
                    # Equation 60 of LA-UR-15-21365
                    bkg_N = sum(bkg_single_feynman_histogram.frequency)
                    bkg_inside_numerator = (2*bkg_second_factorial_moment**2/bkg_first_factorial_moment**2)+(bkg_second_factorial_moment**3/bkg_first_factorial_moment**4)+bkg_second_factorial_moment-bkg_first_factorial_moment**2+(bkg_fourth_factorial_moment/bkg_first_factorial_moment**2)-(2*bkg_second_factorial_moment*bkg_third_factorial_moment)/bkg_first_factorial_moment**3-2*bkg_third_factorial_moment/bkg_first_factorial_moment
                    if bkg_inside_numerator > 0:
                        bkg_dYm = (bkg_inside_numerator/(bkg_N-1))**0.5
                    else:
                        bkg_dYm.append(0)
                        print('Note: dYm was requiring a negative square root. dYm has been set to 0 (but it is not correct).')
                    bkg_dYm_list.append(bkg_dYm)
                    ...
                    if plot_histograms == True or sum_Feynman_histograms == True:
                        ...
                        current_save_path = output_subdirectory_paths(output_nested_subdirectories, save_path, 'Individual_Files/Histograms/', 'Histogram_', bkg_subtract_fname, add_current_file=True, make_dirs=True)
                        if (plot_histograms == True and sum_Feynman_histograms != True) or (plot_histograms == True and sum_Feynman_histograms == True and plot_sum_and_individual_histogram == True):
                            bkg_single_feynman_figure, bkg_single_feynman_axis, bkg_x, bkg_y, bkg_distribution = bkg_single_feynman_histogram.plotHistogram(poisson=True, show=False, limit_step=True, save=current_save_path, log=True, xmin=False, xmax=False, ymin=False, ymax=False, normalize=histogram_normalize, title=str(gatewidth)+' nsec')
                        else:
                            bkg_single_feynman_figure, bkg_single_feynman_axis, bkg_x, bkg_y, bkg_distribution = bkg_single_feynman_histogram.plotHistogram(poisson=True, show=False, limit_step=True, save=False, log=True, xmin=False, xmax=False, ymin=False, ymax=False, normalize=histogram_normalize, title=str(gatewidth)+' nsec')
                        bkg_histogram_x_list.append(bkg_x)
                        bkg_histogram_y_list.append(bkg_y)
                        bkg_histogram_P_list.append(bkg_distribution)
                        if histogram_normalize == True:
                            bkg_y_normalize_no_zero = [[ele for ele in sub if ele != 0] for sub in bkg_histogram_y_list]
            
            if input_type == '.xml':
                
                bkg_first_reduced_factorial_moment_list, bkg_second_reduced_factorial_moment_list, bkg_third_reduced_factorial_moment_list, bkg_fourth_reduced_factorial_moment_list, bkg_n_list = reduced_factorial_moments(bkg_feynman_data)
                bkg_first_factorial_moment_list, bkg_second_factorial_moment_list, bkg_third_factorial_moment_list, bkg_fourth_factorial_moment_list = factorial_moments(bkg_feynman_data)
                bkg_single_feynman_mean_list = bkg_first_factorial_moment_list
                bkg_single_feynman_variance_list = bkg_second_factorial_moment_list
                bkg_single_variance_to_mean_list = feynman_histogram_v2m(bkg_single_feynman_mean_list,bkg_single_feynman_variance_list)
                gate_width_sec = [entry*1e-9 for entry in gatewidth_list]
                bkg_Y1_list, bkg_dY1_list, bkg_Y2_list, bkg_dY2_list = excess_variance_reduced(gate_width_sec, bkg_feynman_data, bkg_first_reduced_factorial_moment_list, bkg_second_reduced_factorial_moment_list, bkg_third_reduced_factorial_moment_list, bkg_fourth_reduced_factorial_moment_list)
                bkg_R1_list = bkg_Y1_list
                bkg_dR1_list = bkg_dY1_list
                bkg_Ym_list, bkg_dYm_list = excess_variance(gate_width_sec, bkg_feynman_data, bkg_first_factorial_moment_list, bkg_second_factorial_moment_list, bkg_third_factorial_moment_list, bkg_fourth_factorial_moment_list)
                ...
                if plot_histograms == True or sum_Feynman_histograms == True:
                    ...
                    for l in range(0,len(gatewidth_list)):
                        ...
                        current_x = bkg_n_list[l]
                        current_y = bkg_feynman_data[l]
                        if histogram_normalize == True:
                            current_P = poissiondist.pmf(current_x, bkg_first_factorial_moment_list[l])
                        else:
                            current_P = sum(current_y)*poissiondist.pmf(current_x, bkg_first_factorial_moment_list[l])
                        if (plot_histograms == True and sum_Feynman_histograms != True) or (plot_histograms == True and sum_Feynman_histograms == True and plot_sum_and_individual_histogram == True):
                        ...
                        bkg_histogram_x_list.append(current_x)
                        bkg_histogram_y_list.append(current_y)
                        bkg_histogram_P_list.append(current_P)
            
            
            # this runs the FeynmanYAnalysis.py script and extracts desired parameters
            if input_type == '.lmx':
                bkg_feynman_analysis = FeynmanYAnalysis()
                bkg_feynman_analysis.makeHistograms(FeynmanHistogramCalculator(bkg_events_list), gates_list=gatewidth_list)
                bkg_Y1_distribution = bkg_feynman_analysis.Y1Distribution()
                bkg_R1_distribution_value = bkg_Y1_distribution_value = bkg_Y1_distribution[0]
                bkg_R1_distribution_unc = bkg_Y1_distribution_unc = bkg_Y1_distribution[1]
                bkg_Y2_distribution = bkg_feynman_analysis.Y2Distribution()
                bkg_Y2_distribution_value = bkg_Y2_distribution[0]
                bkg_Y2_distribution_unc = bkg_Y2_distribution[1]
            elif input_type == '.xml':
                bkg_R1_distribution_value = bkg_Y1_distribution_value = tuple(bkg_Y1_list)
                bkg_R1_distribution_unc = bkg_Y1_distribution_unc = tuple(bkg_dY1_list)
                bkg_Y1_distribution = (bkg_Y1_distribution_value,bkg_Y1_distribution_unc)
                bkg_Y2_distribution_value = tuple(bkg_Y2_list)
                bkg_Y2_distribution_unc = tuple(bkg_dY2_list)
                bkg_Y2_distribution = (bkg_Y2_distribution_value,bkg_Y2_distribution_unc)
            
            bkg_R2_list = [0 for element in range(0,len(bkg_R1_list))]
            ...
            if input_type == '.lmx' or input_type == '.xml':
                if use_user_specified_lifetime == True:
                    ...
                    bkg_user_fit_results = curve_fit(log_one_user, gate_width_sec_b, bkg_Y2_list, method="lm")
                    temp = bkg_user_fit_results[0]
                    bkg_user_A = temp[0]
                    temp = bkg_user_fit_results[1]
                    temp = temp[0]
                    bkg_user_A_unc = np.sqrt(temp[0])
            
                    print('Fit with user-specified lifetime. A = ',bkg_user_A,' +/- ',bkg_user_A_unc,'. lifetime (1/B) = ',bkg_user_lifetime,' +/- ',bkg_user_lifetime_unc)
                    ...
                    for gate in fit_gatewidths_b:
                        bkg_fit_Y2.append(log_one_user(gate, bkg_user_A))
                    
                    bkg_fit_lifetime_user_results = []
                    bkg_fit_lifetime_user_results.append(fit_gatewidths)
                    bkg_fit_lifetime_user_results.append(bkg_fit_Y2)
                    bkg_omega2_lifetime_user_results = omega2_single(1/bkg_user_lifetime,gate_width_sec)
                    bkg_R2_user_lifetime = R2_calc_rate(bkg_Y2_distribution_value,bkg_omega2_lifetime_user_results)
                    bkg_R2_unc_user_lifetime = R2_unc_calc(gatewidth_list,bkg_R2_user_lifetime, bkg_omega2_lifetime_user_results, bkg_Y2_distribution_unc, 1/bkg_user_lifetime, bkg_user_lifetime_unc/bkg_user_lifetime**2)
            
        if plot_individual_files == True:
            current_save_path = output_subdirectory_paths(output_nested_subdirectories, save_path, 'Individual_Files/Rates/', '', bkg_subtract_fname, add_current_file=True, make_dirs=False)    
            current_file_description = None
            
            # plots of each parameter vs gatewidth for individual files
            plot_scatter_gatewidth(current_file_description, 'm1', 'micro-sec', gatewidth_list, bkg_first_reduced_factorial_moment_list, marker_size, plot_titles, bkg_subtract_fname[:-4], y_unc=None, x_div=1000, show=False, save=current_save_path, mult_files=False, yaxis_log=yaxis_log)
            plot_scatter_gatewidth(current_file_description, 'm2', 'micro-sec', gatewidth_list, bkg_second_reduced_factorial_moment_list, marker_size, plot_titles, bkg_subtract_fname[:-4], y_unc=None, x_div=1000, show=False, save=current_save_path, mult_files=False, yaxis_log=yaxis_log)
            plot_scatter_gatewidth(current_file_description, 'm3', 'micro-sec', gatewidth_list, bkg_third_reduced_factorial_moment_list, marker_size, plot_titles, bkg_subtract_fname[:-4], y_unc=None, x_div=1000, show=False, save=current_save_path, mult_files=False, yaxis_log=yaxis_log)
            plot_scatter_gatewidth(current_file_description, 'm4', 'micro-sec', gatewidth_list, bkg_fourth_reduced_factorial_moment_list, marker_size, plot_titles, bkg_subtract_fname[:-4], y_unc=None, x_div=1000, show=False, save=current_save_path, mult_files=False, yaxis_log=yaxis_log)
            plot_scatter_gatewidth(current_file_description, 'C1', 'micro-sec', gatewidth_list, bkg_first_factorial_moment_list, marker_size, plot_titles, bkg_subtract_fname[:-4], y_unc=None, x_div=1000, show=False, save=current_save_path, mult_files=False, yaxis_log=yaxis_log)
            plot_scatter_gatewidth(current_file_description, 'C2', 'micro-sec', gatewidth_list, bkg_second_factorial_moment_list, marker_size, plot_titles, bkg_subtract_fname[:-4], y_unc=None, x_div=1000, show=False, save=current_save_path, mult_files=False, yaxis_log=yaxis_log)
            plot_scatter_gatewidth(current_file_description, 'C3', 'micro-sec', gatewidth_list, bkg_third_factorial_moment_list, marker_size, plot_titles, bkg_subtract_fname[:-4], y_unc=None, x_div=1000, show=False, save=current_save_path, mult_files=False, yaxis_log=yaxis_log)
            plot_scatter_gatewidth(current_file_description, 'C4', 'micro-sec', gatewidth_list, bkg_fourth_factorial_moment_list, marker_size, plot_titles, bkg_subtract_fname[:-4], y_unc=None, x_div=1000, show=False, save=current_save_path, mult_files=False, yaxis_log=yaxis_log)
            plot_scatter_gatewidth(current_file_description, 'Y1', 'micro-sec', gatewidth_list, bkg_Y1_list, marker_size, plot_titles, bkg_subtract_fname[:-4], y_unc=bkg_dY1_list, x_div=1000, show=False, save=current_save_path, mult_files=False, yaxis_log=yaxis_log)
            plot_scatter_gatewidth(current_file_description, 'dY1', 'micro-sec', gatewidth_list, bkg_dY1_list, marker_size, plot_titles, bkg_subtract_fname[:-4], y_unc=None, x_div=1000, show=False, save=current_save_path, mult_files=False, yaxis_log=yaxis_log)
            plot_scatter_gatewidth(current_file_description, 'R1', 'micro-sec', gatewidth_list, bkg_R1_list, marker_size, plot_titles, bkg_subtract_fname[:-4], y_unc=bkg_dR1_list, x_div=1000, show=False, save=current_save_path, mult_files=False, yaxis_log=yaxis_log)
            plot_scatter_gatewidth(current_file_description, 'dR1', 'micro-sec', gatewidth_list, bkg_dR1_list, marker_size, plot_titles, bkg_subtract_fname[:-4], y_unc=None, x_div=1000, show=False, save=current_save_path, mult_files=False, yaxis_log=yaxis_log)
            plot_scatter_gatewidth(current_file_description, 'Y2', 'micro-sec', gatewidth_list, bkg_Y2_list, marker_size, plot_titles, bkg_subtract_fname[:-4], y_unc=bkg_dY2_list, x_div=1000, show=False, save=current_save_path, mult_files=False, yaxis_log=yaxis_log)
            plot_scatter_gatewidth(current_file_description, 'dY2', 'micro-sec', gatewidth_list, bkg_dY2_list, marker_size, plot_titles, bkg_subtract_fname[:-4], y_unc=None, x_div=1000, show=False, save=current_save_path, mult_files=False, yaxis_log=yaxis_log)
            plot_scatter_gatewidth(current_file_description, 'Ym', 'micro-sec', gatewidth_list, bkg_Ym_list, marker_size, plot_titles, bkg_subtract_fname[:-4], y_unc=bkg_dYm_list, x_div=1000, show=False, save=current_save_path, mult_files=False, yaxis_log=yaxis_log)
            plot_scatter_gatewidth(current_file_description, 'dYm', 'micro-sec', gatewidth_list, bkg_dYm_list, marker_size, plot_titles, bkg_subtract_fname[:-4], y_unc=None, x_div=1000, show=False, save=current_save_path, mult_files=False, yaxis_log=yaxis_log)
        ...
        if output_csv == True:
            if plot_histograms == True or plot_individual_files == True or plot_all_files_vs_gw == True or plot_all_files_vs_con == True or perform_Y2_single_fit == True or perform_Y2_double_fit == True:
                ...
                with open(csv_name, mode='w', newline='') as csv_file:
                    writer = csv.writer(csv_file)
                    header_row = ['Gate-width','m1','m2','m3','m4','C1','C2','C3','C4','Y1','dY1','Y2','dY2','R1','dR1','Ym','dYm']
                              
                    writer.writerow(header_row)
                    for value in range(len(gatewidth_list)):
                        data_row = [gatewidth_list[value], bkg_first_reduced_factorial_moment_list[value], bkg_second_reduced_factorial_moment_list[value], bkg_third_reduced_factorial_moment_list[value], bkg_fourth_reduced_factorial_moment_list[value], bkg_first_factorial_moment_list[value], bkg_second_factorial_moment_list[value], bkg_third_factorial_moment_list[value], bkg_fourth_factorial_moment_list[value], bkg_Y1_list[value], bkg_dY1_list[value], bkg_Y2_list[value], bkg_dY2_list[value], bkg_R1_list[value], bkg_dR1_list[value], bkg_Ym_list[value], bkg_dYm_list[value]]
                        writer.writerow(data_row)

# end BKG file

# Start foreground files
if plot_histograms == True or plot_individual_files == True or plot_all_files_vs_gw == True or plot_all_files_vs_con == True or plot_channels == True or perform_Rossi == True or read_header == True:
    os.chdir(filepath)
    ...
    for i in range(0,len(files)):
        ...
        if plot_channels == True:
            ...
            if input_type == '.lmx':
                ...
                channel_plot(detector_ct_file,current_save_path, use_title, title_prefix,channel_display)
        ...
        if input_type == '.lmx':
            ...
            # gets every event in the file and puts in a list
            events_list = []
            #[events_list.extend(ReadToEvents(current_file))]
        ...
        if plot_histograms == True or plot_individual_files == True or plot_all_files_vs_gw == True or plot_all_files_vs_con == True or perform_Y2_single_fit == True or perform_Y2_double_fit == True:
            ...
            # perform Y2 randomized fitting
            if Y2_fit_randomized_data == True:
            
                if type(Y2_fit_randomized_data_number_points) == int:
                    if Y2_fit_randomized_data_number_points > len(gatewidth_list):
                        sys.exit('Exit: Y2_fit_randomized_data_number_points is greater than the number of gate-widths.')
            
                all_samples = []
                gatewidth_all_samples = [gatewidth_list]
                Y2_all_samples = [tuple(Y2_list)]
                dY2_all_samples = [tuple(dY2_list)]
                
                for t in range(0,Y2_fit_randomized_data_number_fits):
                    if Y2_fit_randomized_data_number_points == 'automatic':
                        # 6 is used as the minimum length
                        sample = np.random.randint(6,len(gatewidth_list))
                    else:
                        sample = Y2_fit_randomized_data_number_points
                    ...
                    s = 0
                    while s < sample:
                        index = np.random.randint(0,len(gatewidth_list))
                        index_list.append(index)
                        if index not in index_list[:-1]:
                            kept_index_list.append(index)
                            s += 1
                   
                    kept_index_list = sorted(kept_index_list)
                    
                    for current_index in kept_index_list:
                        new_gatewidth_list.append(gatewidth_list[current_index])
                        new_Y2_list.append(Y2_list[current_index])
                        new_dY2_list.append(dY2_list[current_index])
                    new_Y2_list = tuple(new_Y2_list)
                    new_dY2_list = tuple(new_dY2_list)
                    gatewidth_all_samples.append(new_gatewidth_list)
                    Y2_all_samples.append(new_Y2_list)
                    dY2_all_samples.append(new_dY2_list)
                    
              
            if Y2_fit_randomized_data != True:
                gatewidth_all_samples = [gatewidth_list]
                Y2_all_samples = [Y2_distribution_value]
                dY2_all_samples = [Y2_distribution_unc]
                
            
            # Y2 single log fit
            if perform_Y2_single_fit == True:
                fit1log_A_random = []
                fit1log_A_unc_random = []
                fit1log_B_random = []
                fit1log_B_unc_random = []
                fit1log_det_lifetime_random = []
                fit1log_det_lifetime_unc_random = []
            
                for s in range(0,len(gatewidth_all_samples)):
                    
                    current_gatewidth = gatewidth_all_samples[s]
                    current_Y2 = Y2_all_samples[s]
                    currentdY2 = dY2_all_samples[s]
                    print('Y2 single fit. Sample number ',s+1,' of',len(gatewidth_all_samples),'. used ',len(current_gatewidth),' data points.')
                
                    fit1log = fit1Log_xml(current_gatewidth, current_Y2, currentdY2, guess=None)
                    ...
                    if s== 0:
                        fit1log_A_temp = fit1log_A
                        fit1log_A_unc_temp = fit1log_A_unc
                        fit1log_B_temp = fit1log_B
                        fit1log_B_unc_temp = fit1log_B_unc
                        fit1log_det_lifetime_temp = fit1log_det_lifetime
                        fit1log_det_lifetime_unc_temp = fit1log_det_lifetime_unc
                    else:
                        fit1log_A_random.append(fit1log_A)
                        fit1log_A_unc_random.append(fit1log_A_unc)
                        fit1log_B_random.append(fit1log_B)
                        fit1log_B_unc_random.append(fit1log_B_unc)
                        fit1log_det_lifetime_random.append(fit1log_det_lifetime)
                        fit1log_det_lifetime_unc_random.append(fit1log_det_lifetime_unc)
                
                fit1log_A = fit1log_A_temp
                fit1log_B = fit1log_B_temp
                fit1log_det_lifetime = fit1log_det_lifetime_temp
                if Y2_fit_randomized_data == True:
                    fit1log_A_unc = np.max([fit1log_A_unc_temp,np.std(fit1log_A_random, ddof=1)])
                    fit1log_B_unc = np.max([fit1log_B_unc_temp,np.std(fit1log_B_random, ddof=1)])
                    fit1log_det_lifetime_unc = np.max([fit1log_det_lifetime_unc_temp,np.std(fit1log_det_lifetime_random, ddof=1)])
                    print('Y2_fit_randomized_data results: Fit with 1 log. A unc went from ',fit1log_A_unc_temp,' to ',fit1log_A_unc,'. B unc went from ',fit1log_B_unc_temp,' to ',fit1log_B_unc,'. Lifetime unc went from ',fit1log_det_lifetime_unc_temp,' to ',fit1log_det_lifetime_unc)
                else:
                    fit1log_A_unc = fit1log_A_unc_temp
                    fit1log_B_unc = fit1log_B_unc_temp
                    fit1log_det_lifetime_unc = fit1log_det_lifetime_unc_temp
                ...
                R2_single_Y2_decay = R2_calc_rate(Y2_list,omega2_single_results)
                R2_unc_single_Y2_decay = R2_unc_calc(gatewidth_list,R2_single_Y2_decay, omega2_single_results, dY2_list, fit1log_B, fit1log_B_unc)
                ...
                if perform_Y2_double_fit == True:
                    ...
                    fit2log_A_random = []
                    fit2log_A_unc_random = []
                    fit2log_B_random = []
                    fit2log_B_unc_random = []
                    fit2log_det_lifetime1_random = []
                    fit2log_det_lifetime1_unc_random = []
                    fit2log_C_random = []
                    fit2log_C_unc_random = []
                    fit2log_D_random = []
                    fit2log_D_unc_random = []
                    fit2log_det_lifetime2_random = []
                    fit2log_det_lifetime2_unc_random = []
                    ...
                    for s in range(0,len(gatewidth_all_samples)):
                        ...
                        current_Y2 = Y2_all_samples[s]
                        currentdY2 = dY2_all_samples[s]
                        print('Y2 double fit. Sample number ',s+1,' of',len(gatewidth_all_samples),'. used ',len(current_gatewidth),' data points.')
                    
                        fit2log = fit2Log_xml(current_gatewidth, current_Y2, currentdY2, guess=None)
                        ...
                        if perform_Y2_double_fit_continue == True:    
                            ...
                            if s== 0:
                                fit2log_A_temp = fit2log_A
                                fit2log_A_unc_temp = fit2log_A_unc
                                fit2log_B_temp = fit2log_B
                                fit2log_B_unc_temp = fit2log_B_unc
                                fit2log_det_lifetime1_temp = fit2log_det_lifetime1
                                fit2log_det_lifetime1_unc_temp = fit2log_det_lifetime1_unc
                                fit2log_C_temp = fit2log_C
                                fit2log_C_unc_temp = fit2log_C_unc
                                fit2log_D_temp = fit2log_D
                                fit2log_D_unc_temp = fit2log_D_unc
                                fit2log_det_lifetime2_temp = fit2log_det_lifetime2
                                fit2log_det_lifetime2_unc_temp = fit2log_det_lifetime2_unc
                            else:
                                fit2log_A_random.append(fit2log_A)
                                fit2log_A_unc_random.append(fit2log_A_unc)
                                fit2log_B_random.append(fit2log_B)
                                fit2log_B_unc_random.append(fit2log_B_unc)
                                fit2log_det_lifetime1_random.append(fit2log_det_lifetime1)
                                fit2log_det_lifetime1_unc_random.append(fit2log_det_lifetime1_unc)
                                fit2log_C_random.append(fit2log_B)
                                fit2log_C_unc_random.append(fit2log_B_unc)
                                fit2log_D_random.append(fit2log_D)
                                fit2log_D_unc_random.append(fit2log_D_unc)
                                fit2log_det_lifetime2_random.append(fit2log_det_lifetime2)
                                fit2log_det_lifetime2_unc_random.append(fit2log_det_lifetime2_unc)
                            
                    fit2log_A = fit2log_A_temp
                    fit2log_B = fit2log_B_temp
                    fit2log_det_lifetime1 = fit2log_det_lifetime1_temp
                    fit2log_C = fit2log_C_temp
                    fit2log_D = fit2log_D_temp
                    fit2log_det_lifetime2 = fit2log_det_lifetime2_temp
                    
                    if Y2_fit_randomized_data == True:
                        fit2log_A_unc = np.max([fit2log_A_unc_temp,np.std(fit2log_A_random, ddof=1)])
                        fit2log_B_unc = np.max([fit2log_B_unc_temp,np.std(fit2log_B_random, ddof=1)])
                        fit2log_det_lifetime1_unc = np.max([fit2log_det_lifetime1_unc_temp,np.std(fit2log_det_lifetime1_random, ddof=1)])
                        fit2log_C_unc = np.max([fit2log_C_unc_temp,np.std(fit2log_C_random, ddof=1)])
                        fit2log_D_unc = np.max([fit2log_D_unc_temp,np.std(fit2log_D_random, ddof=1)])
                        fit2log_det_lifetime2_unc = np.max([fit2log_det_lifetime2_unc_temp,np.std(fit2log_det_lifetime2_random, ddof=1)])
                        print('Y2_fit_randomized_data results: Fit with 2 log. A unc went from ',fit2log_A_unc_temp,' to ',fit2log_A_unc,'. B unc went from ',fit2log_B_unc_temp,' to ',fit2log_B_unc,'C unc went from ',fit2log_C_unc_temp,' to ',fit2log_C_unc,'. D unc went from ',fit2log_D_unc_temp,' to ',fit2log_D_unc,'. Lifetime 1 unc went from ',fit2log_det_lifetime1_unc_temp,' to ',fit2log_det_lifetime1_unc,'. Lifetime 2 unc went from ',fit2log_det_lifetime2_unc_temp,' to ',fit2log_det_lifetime2_unc)
                    else:
                        fit2log_A_unc = fit2log_A_unc_temp
                        fit2log_B_unc = fit2log_B_unc_temp
                        fit2log_det_lifetime1_unc = fit2log_det_lifetime1_unc_temp
                        fit2log_C_unc = fit2log_C_unc_temp
                        fit2log_D_unc = fit2log_D_unc_temp
                        fit2log_det_lifetime2_unc = fit2log_det_lifetime2_unc_temp
                    ...
                    if perform_Y2_double_fit_continue == True:    
                        ...
                        R2_double1_Y2_decay = R2_calc_rate(Y2_list,omega2_double1_results)
                        R2_unc_double1_Y2_decay = R2_unc_calc(gatewidth_list,R2_double1_Y2_decay, omega2_double1_results, dY2_list, fit2log_B, fit2log_B_unc)
                        omega2_double2_results = omega2_single(fit2log_D,gatewidth_list)
                        R2_double2_Y2_decay = R2_calc_rate(Y2_list,omega2_double2_results)
                        R2_unc_double2_Y2_decay = R2_unc_calc(gatewidth_list,R2_double2_Y2_decay, omega2_double2_results, dY2_list, fit2log_D, fit2log_D_unc)
                        ...
                        R2_double_both_Y2_decay = R2_calc_rate(Y2_list,omega2_double_both_results)
                        R2_unc_double_both_Y2_decay = R2_double_unc_calc(gatewidth_list,R2_double_both_Y2_decay, omega2_double_both_results, dY2_list, fit2logA_percent, fit2log_B, fit2log_B_unc, fit2logC_percent, fit2log_D, fit2log_D_unc, fit2log_B_D)
            ...
            # performs BKG subtraction
            if perform_bkg_subtract_rates == True:
                if len(R1_list) != len(bkg_R1_list):
                    sys.exit('Exit: length of foreground and background were different.')
                else:
                    print('R1_list prior to bkg subtraction: ',R1_list)
                    print('BKG R1 ',bkg_R1_list)
                    for o in range(0,len(R1_list)):
                        R1_list[o] = R1_list[o]-bkg_R1_list[o]
                        dR1_list[o] = (dR1_list[o]**2+bkg_dR1_list[o]**2)**0.5
                    print('R1_list after bkg subtraction: ',R1_list)
                    if np.min(R1_list) < 0:
                        print('NOTE: BKG subtraction resulted in negative rates. BKG has higher count rate than foreground.')
        ...
        if plot_individual_files == True:
            ...
            if perform_Y2_single_fit == True:
                ...
                if Y2_fit_randomized_data == True:
                    plotHistogram_vs_gaussian('Single Fit A', gatewidth_all_samples, fit1log_A_random, fit1log_A, np.std(fit1log_A_random, ddof=1), [], show=False, limit_step=False, save=current_save_path+'_single_fit', log=False, title=False, show_gaussian=True, show_avg=True, show_sd=True, show_unc_avg=False)
                    plotHistogram_vs_gaussian('Single Fit B', gatewidth_all_samples, fit1log_B_random, fit1log_B, np.std(fit1log_B_random, ddof=1), [], show=False, limit_step=False, save=current_save_path+'_single_fit', log=False, title=False, show_gaussian=True, show_avg=True, show_sd=True, show_unc_avg=False)
                    plotHistogram_vs_gaussian('Single Fit Lifetime', gatewidth_all_samples, fit1log_det_lifetime_random, fit1log_det_lifetime, np.std(fit1log_det_lifetime_random, ddof=1), [], show=False, limit_step=False, save=current_save_path+'_single_fit', log=False, title=False, show_gaussian=True, show_avg=True, show_sd=True, show_unc_avg=False)
            ...
            if perform_Y2_double_fit == True and perform_Y2_double_fit_continue == True:
                ...
                if Y2_fit_randomized_data == True:
                    plotHistogram_vs_gaussian('Double Fit A', gatewidth_all_samples, fit2log_A_random, fit2log_A, np.std(fit2log_A_random, ddof=1), [], show=False, limit_step=False, save=current_save_path+'_double_fit', log=False, title=False, show_gaussian=True, show_avg=True, show_sd=True, show_unc_avg=False)
                    plotHistogram_vs_gaussian('Double Fit B', gatewidth_all_samples, fit2log_B_random, fit2log_B, np.std(fit2log_B_random, ddof=1), [], show=False, limit_step=False, save=current_save_path+'_double_fit', log=False, title=False, show_gaussian=True, show_avg=True, show_sd=True, show_unc_avg=False)
                    plotHistogram_vs_gaussian('Double Fit Lifetime 1', gatewidth_all_samples, fit2log_det_lifetime1_random, fit2log_det_lifetime1, np.std(fit2log_det_lifetime1_random, ddof=1), [], show=False, limit_step=False, save=current_save_path+'_double_fit', log=False, title=False, show_gaussian=True, show_avg=True, show_sd=True, show_unc_avg=False)
                    plotHistogram_vs_gaussian('Double Fit C', gatewidth_all_samples, fit2log_C_random, fit2log_C, np.std(fit2log_C_random, ddof=1), [], show=False, limit_step=False, save=current_save_path+'_double_fit', log=False, title=False, show_gaussian=True, show_avg=True, show_sd=True, show_unc_avg=False)
                    plotHistogram_vs_gaussian('Double Fit D', gatewidth_all_samples, fit2log_D_random, fit2log_D, np.std(fit2log_D_random, ddof=1), [], show=False, limit_step=False, save=current_save_path+'_double_fit', log=False, title=False, show_gaussian=True, show_avg=True, show_sd=True, show_unc_avg=False)
                    plotHistogram_vs_gaussian('Double Fit Lifetime 2', gatewidth_all_samples, fit2log_det_lifetime2_random, fit2log_det_lifetime2, np.std(fit2log_det_lifetime2_random, ddof=1), [], show=False, limit_step=False, save=current_save_path+'_double_fit', log=False, title=False, show_gaussian=True, show_avg=True, show_sd=True, show_unc_avg=False)
    ...
    if len(files) > 1:
        ...
        if plot_all_files_vs_con == True:
            ...
            for k in range(0,len(gatewidth_transposed)):
                ...
                if combine_statistical_plots != True:
                    if user_gatewidths_plot_all_files_vs_con != True or (user_gatewidths_plot_all_files_vs_con == True and current_gatewidth[0] in plot_all_files_vs_con_gatewidth_list):
                        plot_scatter('Configuration', 'm1', config_x_label, current_m1, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                        plot_scatter('Configuration', 'm2', config_x_label, current_m2, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                        plot_scatter('Configuration', 'm3', config_x_label, current_m3, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                        plot_scatter('Configuration', 'm4', config_x_label, current_m4, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                        plot_scatter('Configuration', 'C1', config_x_label, current_C1, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                        plot_scatter('Configuration', 'C2', config_x_label, current_C2, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                        plot_scatter('Configuration', 'C3', config_x_label, current_C3, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                        plot_scatter('Configuration', 'C4', config_x_label, current_C4, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                        plot_scatter('Configuration', 'Y1', config_x_label, current_Y1, marker_size, y_unc=current_dY1, show=False, save=current_save_path+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                        plot_scatter('Configuration', 'dY1', config_x_label, current_dY1, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                        plot_scatter('Configuration', 'R1', config_x_label, current_R1, marker_size, y_unc=current_dR1, show=False, save=current_save_path+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                        plot_scatter('Configuration', 'dR1', config_x_label, current_dR1, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                        plot_scatter('Configuration', 'Y2', config_x_label, current_Y2, marker_size, y_unc=current_dY2, show=False, save=current_save_path+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                        plot_scatter('Configuration', 'dY2', config_x_label, current_dY2, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                        plot_scatter('Configuration', 'Ym', config_x_label, current_Ym, marker_size, y_unc=current_dYm, show=False, save=current_save_path+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                        plot_scatter('Configuration', 'dYm', config_x_label, current_dYm, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                        if perform_Y2_single_fit == True:
                            if input_type != '.csv':
                                plot_scatter('Configuration', 'omega2', config_x_label, current_omega2_single, marker_size, y_unc=None, show=False, save=current_save_path+'_single'+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                            plot_scatter('Configuration', 'R2', config_x_label, current_R2_single_Y2_decay, marker_size, y_unc=current_R2_unc_single_Y2_decay, show=False, save=current_save_path+'_single'+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                            plot_scatter('Configuration', 'dR2', config_x_label, current_R2_unc_single_Y2_decay, marker_size, y_unc=None, show=False, save=current_save_path+'_single'+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                        if perform_Y2_double_fit == True and perform_Y2_double_fit_continue == True:
                            if input_type != '.csv':
                                plot_scatter('Configuration', 'omega2', config_x_label, current_omega2_double1, marker_size, y_unc=None, show=False, save=current_save_path+'_double1'+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                                plot_scatter('Configuration', 'omega2', config_x_label, current_omega2_double2, marker_size, y_unc=None, show=False, save=current_save_path+'_double2'+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                            plot_scatter('Configuration', 'R2', config_x_label, current_R2_double1_Y2_decay, marker_size, y_unc=current_R2_unc_double1_Y2_decay, show=False, save=current_save_path+'_double1'+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                            plot_scatter('Configuration', 'dR2', config_x_label, current_R2_unc_double1_Y2_decay, marker_size, y_unc=None, show=False, save=current_save_path+'_double1'+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                            plot_scatter('Configuration', 'R2', config_x_label, current_R2_double2_Y2_decay, marker_size, y_unc=current_R2_unc_double2_Y2_decay, show=False, save=current_save_path+'_double2'+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                            plot_scatter('Configuration', 'dR2', config_x_label, current_R2_unc_double2_Y2_decay, marker_size, y_unc=None, show=False, save=current_save_path+'_double2'+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                            plot_scatter('Configuration', 'R2', config_x_label, current_R2_double_both_Y2_decay, marker_size, y_unc=current_R2_unc_double_both_Y2_decay, show=False, save=current_save_path+'_double_both'+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                            plot_scatter('Configuration', 'dR2', config_x_label, current_R2_unc_double_both_Y2_decay, marker_size, y_unc=None, show=False, save=current_save_path+'_double_both'+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                        if use_user_specified_lifetime == True:
                            if input_type != '.csv':
                                plot_scatter('Configuration', 'omega2', config_x_label, current_omega2_lifetime_user, marker_size, y_unc=None, show=False, save=current_save_path+'_user_lifetime'+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                            plot_scatter('Configuration', 'R2', config_x_label, current_R2_user_lifetime, marker_size, y_unc=current_R2_unc_user_lifetime, show=False, save=current_save_path+'_user_lifetime'+str(current_gatewidth[0]), yaxis_log=yaxis_log)
                            plot_scatter('Configuration', 'dR2', config_x_label, current_R2_unc_user_lifetime, marker_size, y_unc=None, show=False, save=current_save_path+'_user_lifetime'+str(current_gatewidth[0]), yaxis_log=yaxis_log)  
                        if calculation_type == 'Cf':
                            plot_scatter('Configuration', 'Efficiency', config_x_label, current_calc_eff_kn, marker_size, y_unc=current_calc_eff_unc_kn, show=False, save=current_save_path+str(current_gatewidth[0]))
                            plot_scatter('Configuration', 'dEfficiency', config_x_label, current_calc_eff_unc_kn, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0]))
                        elif calculation_type == 'Multiplicity':
                            if perform_Y2_single_fit == True:
                                plot_scatter('Configuration', 'Ml', config_x_label, current_Ml_single, marker_size, y_unc=current_dMl_single, show=False, save=current_save_path+str(current_gatewidth[0])+'_single_fit')
                                plot_scatter('Configuration', 'dMl', config_x_label, current_dMl_single, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0])+'_single_fit')
                                plot_scatter('Configuration', 'Fs', config_x_label, current_Fs_single, marker_size, y_unc=current_dFs_single, show=False, save=current_save_path+str(current_gatewidth[0])+'_single_fit')
                                plot_scatter('Configuration', 'dFs', config_x_label, current_dFs_single, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0])+'_single_fit')
                                plot_scatter('Configuration', 'Mt', config_x_label, current_Mt_single, marker_size, y_unc=current_dMt_single, show=False, save=current_save_path+str(current_gatewidth[0])+'_single_fit')
                                plot_scatter('Configuration', 'dMt', config_x_label, current_dMt_single, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0])+'_single_fit')
                                plot_scatter('Configuration', 'kp', config_x_label, current_kp_single, marker_size, y_unc=current_dkp_single, show=False, save=current_save_path+str(current_gatewidth[0])+'_single_fit')
                                plot_scatter('Configuration', 'dkp', config_x_label, current_dkp_single, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0])+'_single_fit')
                                plot_scatter('Configuration', 'keff', config_x_label, current_keff_single, marker_size, y_unc=current_dkeff_single, show=False, save=current_save_path+str(current_gatewidth[0])+'_single_fit')
                                plot_scatter('Configuration', 'dkeff', config_x_label, current_dkeff_single, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0])+'_single_fit')
                            if perform_Y2_double_fit == True and perform_Y2_double_fit_continue == True:
                                plot_scatter('Configuration', 'Ml', config_x_label, current_Ml_double1, marker_size, y_unc=current_dMl_double1, show=False, save=current_save_path+str(current_gatewidth[0])+'_double1_fit')
                                plot_scatter('Configuration', 'dMl', config_x_label, current_dMl_double1, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0])+'_double1_fit')
                                plot_scatter('Configuration', 'Fs', config_x_label, current_Fs_double1, marker_size, y_unc=current_dFs_double1, show=False, save=current_save_path+str(current_gatewidth[0])+'_double1_fit')
                                plot_scatter('Configuration', 'dFs', config_x_label, current_dFs_double1, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0])+'_double1_fit')
                                plot_scatter('Configuration', 'Mt', config_x_label, current_Mt_double1, marker_size, y_unc=current_dMt_double1, show=False, save=current_save_path+str(current_gatewidth[0])+'_double1_fit')
                                plot_scatter('Configuration', 'dMt', config_x_label, current_dMt_double1, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0])+'_double1_fit')
                                plot_scatter('Configuration', 'kp', config_x_label, current_kp_double1, marker_size, y_unc=current_dkp_double1, show=False, save=current_save_path+str(current_gatewidth[0])+'_double1_fit')
                                plot_scatter('Configuration', 'dkp', config_x_label, current_dkp_double1, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0])+'_double1_fit')
                                plot_scatter('Configuration', 'keff', config_x_label, current_keff_double1, marker_size, y_unc=current_dkeff_double1, show=False, save=current_save_path+str(current_gatewidth[0])+'_double1_fit')
                                plot_scatter('Configuration', 'dkeff', config_x_label, current_dkeff_double1, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0])+'_double1_fit')
                                plot_scatter('Configuration', 'Ml', config_x_label, current_Ml_double2, marker_size, y_unc=current_dMl_double2, show=False, save=current_save_path+str(current_gatewidth[0])+'_double2_fit')
                                plot_scatter('Configuration', 'dMl', config_x_label, current_dMl_double2, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0])+'_double2_fit')
                                plot_scatter('Configuration', 'Fs', config_x_label, current_Fs_double2, marker_size, y_unc=current_dFs_double2, show=False, save=current_save_path+str(current_gatewidth[0])+'_double2_fit')
                                plot_scatter('Configuration', 'dFs', config_x_label, current_dFs_double2, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0])+'_double2_fit')
                                plot_scatter('Configuration', 'Mt', config_x_label, current_Mt_double2, marker_size, y_unc=current_dMt_double2, show=False, save=current_save_path+str(current_gatewidth[0])+'_double2_fit')
                                plot_scatter('Configuration', 'dMt', config_x_label, current_dMt_double2, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0])+'_double2_fit')
                                plot_scatter('Configuration', 'kp', config_x_label, current_kp_double2, marker_size, y_unc=current_dkp_double2, show=False, save=current_save_path+str(current_gatewidth[0])+'_double2_fit')
                                plot_scatter('Configuration', 'dkp', config_x_label, current_dkp_double2, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0])+'_double2_fit')
                                plot_scatter('Configuration', 'keff', config_x_label, current_keff_double2, marker_size, y_unc=current_dkeff_double2, show=False, save=current_save_path+str(current_gatewidth[0])+'_double2_fit')
                                plot_scatter('Configuration', 'dkeff', config_x_label, current_dkeff_double2, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0])+'_double2_fit')
                                plot_scatter('Configuration', 'Ml', config_x_label, current_Ml_double_both, marker_size, y_unc=current_dMl_double_both, show=False, save=current_save_path+str(current_gatewidth[0])+'_double_both_fit')
                                plot_scatter('Configuration', 'dMl', config_x_label, current_dMl_double_both, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0])+'_double_both_fit')
                                plot_scatter('Configuration', 'Fs', config_x_label, current_Fs_double_both, marker_size, y_unc=current_dFs_double_both, show=False, save=current_save_path+str(current_gatewidth[0])+'_double_both_fit')
                                plot_scatter('Configuration', 'dFs', config_x_label, current_dFs_double_both, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0])+'_double_both_fit')
                                plot_scatter('Configuration', 'Mt', config_x_label, current_Mt_double_both, marker_size, y_unc=current_dMt_double_both, show=False, save=current_save_path+str(current_gatewidth[0])+'_double_both_fit')
                                plot_scatter('Configuration', 'dMt', config_x_label, current_dMt_double_both, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0])+'_double_both_fit')
                                plot_scatter('Configuration', 'kp', config_x_label, current_kp_double_both, marker_size, y_unc=current_dkp_double_both, show=False, save=current_save_path+str(current_gatewidth[0])+'_double_both_fit')
                                plot_scatter('Configuration', 'dkp', config_x_label, current_dkp_double_both, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0])+'_double_both_fit')
                                plot_scatter('Configuration', 'keff', config_x_label, current_keff_double_both, marker_size, y_unc=current_dkeff_double_both, show=False, save=current_save_path+str(current_gatewidth[0])+'_double_both_fit')
                                plot_scatter('Configuration', 'dkeff', config_x_label, current_dkeff_double_both, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0])+'_double_both_fit')                      
                            if use_user_specified_lifetime == True:
                                plot_scatter('Configuration', 'Ml', config_x_label, current_Ml_user_lifetime, marker_size, y_unc=current_dMl_user_lifetime, show=False, save=current_save_path+str(current_gatewidth[0])+'_user_lifetime_fit')
                                plot_scatter('Configuration', 'dMl', config_x_label, current_dMl_user_lifetime, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0])+'_user_lifetime_fit')
                                plot_scatter('Configuration', 'Fs', config_x_label, current_Fs_user_lifetime, marker_size, y_unc=current_dFs_user_lifetime, show=False, save=current_save_path+str(current_gatewidth[0])+'_user_lifetime_fit')
                                plot_scatter('Configuration', 'dFs', config_x_label, current_dFs_user_lifetime, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0])+'_user_lifetime_fit')
                                plot_scatter('Configuration', 'Mt', config_x_label, current_Mt_user_lifetime, marker_size, y_unc=current_dMt_user_lifetime, show=False, save=current_save_path+str(current_gatewidth[0])+'_user_lifetime_fit')
                                plot_scatter('Configuration', 'dMt', config_x_label, current_dMt_user_lifetime, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0])+'_user_lifetime_fit')
                                plot_scatter('Configuration', 'kp', config_x_label, current_kp_user_lifetime, marker_size, y_unc=current_dkp_user_lifetime, show=False, save=current_save_path+str(current_gatewidth[0])+'_user_lifetime_fit')
                                plot_scatter('Configuration', 'dkp', config_x_label, current_dkp_user_lifetime, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0])+'_user_lifetime_fit')
                                plot_scatter('Configuration', 'keff', config_x_label, current_keff_user_lifetime, marker_size, y_unc=current_dkeff_user_lifetime, show=False, save=current_save_path+str(current_gatewidth[0])+'_user_lifetime_fit')
                                plot_scatter('Configuration', 'dkeff', config_x_label, current_dkeff_user_lifetime, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth[0])+'_user_lifetime_fit')
            ...
            if input_type != '.csv':
                for k in range(0,len(file_number)):
                    if perform_Y2_single_fit == True:
                        plot_scatter('Configuration', 'A', config_x_label, fit1log_A_all_files, marker_size, y_unc=fit1log_A_unc_all_files, show=False, save=current_save_path+'_Y2_fit1_', yaxis_log=yaxis_log)
                        plot_scatter('Configuration', 'B', config_x_label, fit1log_B_all_files, marker_size, y_unc=fit1log_B_unc_all_files, show=False, save=current_save_path+'_Y2_fit1_', yaxis_log=yaxis_log)
                        plot_scatter('Configuration', 'lambda', config_x_label, fit1log_det_lifetime_all_files, marker_size, y_unc=fit1log_det_lifetime_unc_all_files, show=False, save=current_save_path+'_Y2_fit1_', yaxis_log=yaxis_log)
                    if perform_Y2_double_fit == True and perform_Y2_double_fit_continue == True:
                        plot_scatter('Configuration', 'A', config_x_label, fit2log_A_all_files, marker_size, y_unc=fit2log_A_unc_all_files, show=False, save=current_save_path+'_Y2_fit2_', yaxis_log=yaxis_log)
                        plot_scatter('Configuration', 'B', config_x_label, fit2log_B_all_files, marker_size, y_unc=fit2log_B_unc_all_files, show=False, save=current_save_path+'_Y2_fit2_', yaxis_log=yaxis_log)
                        plot_scatter('Configuration', 'lambda1', config_x_label, fit2log_det_lifetime1_all_files, marker_size, y_unc=fit2log_det_lifetime1_unc_all_files, show=False, save=current_save_path+'_Y2_fit2_', yaxis_log=yaxis_log)
                        plot_scatter('Configuration', 'C', config_x_label, fit2log_C_all_files, marker_size, y_unc=fit2log_C_unc_all_files, show=False, save=current_save_path+'_Y2_fit2_', yaxis_log=yaxis_log)
                        plot_scatter('Configuration', 'D', config_x_label, fit2log_D_all_files, marker_size, y_unc=fit2log_D_unc_all_files, show=False, save=current_save_path+'_Y2_fit2_', yaxis_log=yaxis_log)
                        plot_scatter('Configuration', 'lambda2', config_x_label, fit2log_det_lifetime2_all_files, marker_size, y_unc=fit2log_det_lifetime2_unc_all_files, show=False, save=current_save_path+'_Y2_fit2_', yaxis_log=yaxis_log)
            ...
            if combine_statistical_plots == True:
                ...
                for gate in range(0,len(gatewidth_list)):
                    ...
                    if user_gatewidths_plot_all_files_vs_con != True or (user_gatewidths_plot_all_files_vs_con == True and current_gatewidth in plot_all_files_vs_con_gatewidth_list):
                        ...
                        #if plot_all_files_vs_con == True:
                        current_save_path = output_subdirectory_paths(output_nested_subdirectories, save_path, 'All_Files/Configurations/', '', current_file, add_current_file=False, make_dirs=True)
                        plot_scatter_with_stats('Configuration', 'm1', config_x_label, current_m1, np.average(current_m1), np.std(current_m1, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth), yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                        plot_scatter_with_stats('Configuration', 'm2', config_x_label, current_m2, np.average(current_m2), np.std(current_m2, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth), yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                        plot_scatter_with_stats('Configuration', 'm3', config_x_label, current_m3, np.average(current_m3), np.std(current_m3, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth), yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                        plot_scatter_with_stats('Configuration', 'm4', config_x_label, current_m4, np.average(current_m4), np.std(current_m4, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth), yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                        plot_scatter_with_stats('Configuration', 'C1', config_x_label, current_C1, np.average(current_C1), np.std(current_C1, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth), yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                        plot_scatter_with_stats('Configuration', 'C2', config_x_label, current_C2, np.average(current_C2), np.std(current_C2, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth), yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                        plot_scatter_with_stats('Configuration', 'C3', config_x_label, current_C3, np.average(current_C3), np.std(current_C3, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth), yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                        plot_scatter_with_stats('Configuration', 'C4', config_x_label, current_C4, np.average(current_C4), np.std(current_C4, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth), yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                        plot_scatter_with_stats('Configuration', 'Y1', config_x_label, Y1_data_current_gate, Y1_combined_current_gate, Y1_combined_sd_current_gate, Y1_individual_unc_avg_current_gate, marker_size, y_unc=dY1_data_current_gate, show=False, save=current_save_path+str(current_gatewidth), yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                        plot_scatter_with_stats('Configuration', 'dY1', config_x_label, dY1_data_current_gate, np.average(dY1_data_current_gate), np.std(dY1_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth), yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                        plot_scatter_with_stats('Configuration', 'R1', config_x_label, R1_data_current_gate, R1_combined_current_gate, R1_combined_sd_current_gate, R1_individual_unc_avg_current_gate, marker_size, y_unc=dR1_data_current_gate, show=False, save=current_save_path+str(current_gatewidth), yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                        plot_scatter_with_stats('Configuration', 'dR1', config_x_label, dR1_data_current_gate, np.average(dR1_data_current_gate), np.std(dR1_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth), yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                        plot_scatter_with_stats('Configuration', 'Y2', config_x_label, Y2_data_current_gate, Y2_combined_current_gate, Y2_combined_sd_current_gate, Y2_individual_unc_avg_current_gate, marker_size, y_unc=dY2_data_current_gate, show=False, save=current_save_path+str(current_gatewidth), yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                        plot_scatter_with_stats('Configuration', 'dY2', config_x_label, dY2_data_current_gate, np.average(dY2_data_current_gate), np.std(dY2_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth), yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                        plot_scatter_with_stats('Configuration', 'Ym', config_x_label, Ym_data_current_gate, Ym_combined_current_gate, Ym_combined_sd_current_gate, Ym_individual_unc_avg_current_gate, marker_size, y_unc=dYm_data_current_gate, show=False, save=current_save_path+str(current_gatewidth), yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                        plot_scatter_with_stats('Configuration', 'dYm', config_x_label, dYm_data_current_gate, np.average(dYm_data_current_gate), np.std(dYm_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth), yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                        if combine_Y2_rate_results_old_method == True:
                            if perform_Y2_single_fit == True:
                                plot_scatter_with_stats('Configuration', 'omega2', config_x_label, current_omega2_single, np.average(current_omega2_single), np.std(current_omega2_single, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_single_', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                plot_scatter_with_stats('Configuration', 'R2', config_x_label, R2_single_Y2_decay_data_current_gate, R2_single_Y2_decay_combined_current_gate, R2_single_Y2_decay_combined_sd_current_gate, R2_single_Y2_decay_individual_unc_avg_current_gate, marker_size, y_unc=R2_unc_single_Y2_decay_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_single_', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                plot_scatter_with_stats('Configuration', 'dR2', config_x_label, R2_unc_single_Y2_decay_data_current_gate, np.average(R2_unc_single_Y2_decay_data_current_gate), np.std(R2_unc_single_Y2_decay_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_single_', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                            if perform_Y2_double_fit == True and perform_Y2_double_fit_continue == True:
                                plot_scatter_with_stats('Configuration', 'omega2', config_x_label, current_omega2_double1, np.average(current_omega2_double1), np.std(current_omega2_double1, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_double1_', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                plot_scatter_with_stats('Configuration', 'R2', config_x_label, R2_double1_Y2_decay_data_current_gate, R2_double1_Y2_decay_combined_current_gate, R2_double1_Y2_decay_combined_sd_current_gate, R2_double1_Y2_decay_individual_unc_avg_current_gate, marker_size, y_unc=R2_unc_double1_Y2_decay_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_double1_', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                plot_scatter_with_stats('Configuration', 'dR2', config_x_label, R2_unc_double1_Y2_decay_data_current_gate, np.average(R2_unc_double1_Y2_decay_data_current_gate), np.std(R2_unc_double1_Y2_decay_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_double1_', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                plot_scatter_with_stats('Configuration', 'omega2', config_x_label, current_omega2_double2, np.average(current_omega2_double2), np.std(current_omega2_double2, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_double2_', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                plot_scatter_with_stats('Configuration', 'R2', config_x_label, R2_double2_Y2_decay_data_current_gate, R2_double2_Y2_decay_combined_current_gate, R2_double2_Y2_decay_combined_sd_current_gate, R2_double2_Y2_decay_individual_unc_avg_current_gate, marker_size, y_unc=R2_unc_double2_Y2_decay_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_double2_', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                plot_scatter_with_stats('Configuration', 'dR2', config_x_label, R2_unc_double2_Y2_decay_data_current_gate, np.average(R2_unc_double2_Y2_decay_data_current_gate), np.std(R2_unc_double2_Y2_decay_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_double2_', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                plot_scatter_with_stats('Configuration', 'R2', config_x_label, R2_double_both_Y2_decay_data_current_gate, R2_double_both_Y2_decay_combined_current_gate, R2_double_both_Y2_decay_combined_sd_current_gate, R2_double_both_Y2_decay_individual_unc_avg_current_gate, marker_size, y_unc=R2_unc_double_both_Y2_decay_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_double_both_', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                plot_scatter_with_stats('Configuration', 'dR2', config_x_label, R2_unc_double_both_Y2_decay_data_current_gate, np.average(R2_unc_double_both_Y2_decay_data_current_gate), np.std(R2_unc_double_both_Y2_decay_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_double_both_', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                            if use_user_specified_lifetime == True:
                                plot_scatter_with_stats('Configuration', 'omega2', config_x_label, current_omega2_user_lifetime, np.average(current_omega2_user_lifetime), np.std(current_omega2_user_lifetime, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_user_lifetime_', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                plot_scatter_with_stats('Configuration', 'R2', config_x_label, R2_user_lifetime_data_current_gate, R2_user_lifetime_combined_current_gate, R2_user_lifetime_combined_sd_current_gate, R2_user_lifetime_individual_unc_avg_current_gate, marker_size, y_unc=R2_unc_user_lifetime_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_user_lifetime_', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                plot_scatter_with_stats('Configuration', 'dR2', config_x_label, R2_unc_user_lifetime_data_current_gate, np.average(R2_unc_user_lifetime_data_current_gate), np.std(R2_unc_user_lifetime_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_user_lifetime_', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                            if calculation_type == 'Cf':
                                plot_scatter_with_stats('Configuration', 'Efficiency', config_x_label, calc_eff_kn_current_gate, calc_eff_kn_combined_current_gate, calc_eff_kn_combined_sd_current_gate, calc_eff_kn_individual_unc_current_gate, marker_size, y_unc=calc_eff_unc_kn_current_gate, show=False, save=current_save_path+str(current_gatewidth), yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                plot_scatter_with_stats('Configuration', 'dEfficiency', config_x_label, calc_eff_unc_kn_current_gate, np.average(calc_eff_unc_kn_current_gate), np.std(calc_eff_unc_kn_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth), yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                            elif calculation_type == 'Multiplicity':
                                if perform_Y2_single_fit == True:
                                    plot_scatter_with_stats('Configuration', 'Ml', config_x_label, Ml_single_data_current_gate, Ml_single_combined_current_gate, Ml_single_combined_sd_current_gate, Ml_single_individual_unc_avg_current_gate, marker_size, y_unc=dMl_single_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_single', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                    plot_scatter_with_stats('Configuration', 'dMl', config_x_label, dMl_single_data_current_gate, np.average(dMl_single_data_current_gate), np.std(dMl_single_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_single', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                    plot_scatter_with_stats('Configuration', 'Mt', config_x_label, Mt_single_data_current_gate, Mt_single_combined_current_gate, Mt_single_combined_sd_current_gate, Mt_single_individual_unc_avg_current_gate, marker_size, y_unc=dMt_single_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_single', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                    plot_scatter_with_stats('Configuration', 'dMt', config_x_label, dMt_single_data_current_gate, np.average(dMt_single_data_current_gate), np.std(dMt_single_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_single', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                    plot_scatter_with_stats('Configuration', 'Fs', config_x_label, Fs_single_data_current_gate, Fs_single_combined_current_gate, Fs_single_combined_sd_current_gate, Fs_single_individual_unc_avg_current_gate, marker_size, y_unc=dFs_single_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_single', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                    plot_scatter_with_stats('Configuration', 'dFs', config_x_label, dFs_single_data_current_gate, np.average(dFs_single_data_current_gate), np.std(dFs_single_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_single', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                    plot_scatter_with_stats('Configuration', 'kp', config_x_label, kp_single_data_current_gate, kp_single_combined_current_gate, kp_single_combined_sd_current_gate, kp_single_individual_unc_avg_current_gate, marker_size, y_unc=dkp_single_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_single', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                    plot_scatter_with_stats('Configuration', 'dkp', config_x_label, dkp_single_data_current_gate, np.average(dkp_single_data_current_gate), np.std(dkp_single_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_single', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                    plot_scatter_with_stats('Configuration', 'keff', config_x_label, keff_single_data_current_gate, keff_single_combined_current_gate, keff_single_combined_sd_current_gate, keff_single_individual_unc_avg_current_gate, marker_size, y_unc=dkeff_single_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_single', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                    plot_scatter_with_stats('Configuration', 'dkeff', config_x_label, dkeff_single_data_current_gate, np.average(dkeff_single_data_current_gate), np.std(dkeff_single_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_single', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                if perform_Y2_double_fit == True and perform_Y2_double_fit_continue == True:
                                    plot_scatter_with_stats('Configuration', 'Ml', config_x_label, Ml_double1_data_current_gate, Ml_double1_combined_current_gate, Ml_double1_combined_sd_current_gate, Ml_double1_individual_unc_avg_current_gate, marker_size, y_unc=dMl_double1_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_double1', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                    plot_scatter_with_stats('Configuration', 'dMl', config_x_label, dMl_double1_data_current_gate, np.average(dMl_double1_data_current_gate), np.std(dMl_double1_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_double1', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                    plot_scatter_with_stats('Configuration', 'Mt', config_x_label, Mt_double1_data_current_gate, Mt_double1_combined_current_gate, Mt_double1_combined_sd_current_gate, Mt_double1_individual_unc_avg_current_gate, marker_size, y_unc=dMt_double1_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_double1', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                    plot_scatter_with_stats('Configuration', 'dMt', config_x_label, dMt_double1_data_current_gate, np.average(dMt_double1_data_current_gate), np.std(dMt_double1_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_double1', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                    plot_scatter_with_stats('Configuration', 'Fs', config_x_label, Fs_double1_data_current_gate, Fs_double1_combined_current_gate, Fs_double1_combined_sd_current_gate, Fs_double1_individual_unc_avg_current_gate, marker_size, y_unc=dFs_double1_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_double1', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                    plot_scatter_with_stats('Configuration', 'dFs', config_x_label, dFs_double1_data_current_gate, np.average(dFs_double1_data_current_gate), np.std(dFs_double1_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_double1', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                    plot_scatter_with_stats('Configuration', 'kp', config_x_label, kp_double1_data_current_gate, kp_double1_combined_current_gate, kp_double1_combined_sd_current_gate, kp_double1_individual_unc_avg_current_gate, marker_size, y_unc=dkp_double1_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_double1', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                    plot_scatter_with_stats('Configuration', 'dkp', config_x_label, dkp_double1_data_current_gate, np.average(dkp_double1_data_current_gate), np.std(dkp_double1_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_double1', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                    plot_scatter_with_stats('Configuration', 'keff', config_x_label, keff_double1_data_current_gate, keff_double1_combined_current_gate, keff_double1_combined_sd_current_gate, keff_double1_individual_unc_avg_current_gate, marker_size, y_unc=dkeff_double1_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_double1', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                    plot_scatter_with_stats('Configuration', 'dkeff', config_x_label, dkeff_double1_data_current_gate, np.average(dkeff_double1_data_current_gate), np.std(dkeff_double1_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_double1', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                    plot_scatter_with_stats('Configuration', 'Ml', config_x_label, Ml_double2_data_current_gate, Ml_double2_combined_current_gate, Ml_double2_combined_sd_current_gate, Ml_double2_individual_unc_avg_current_gate, marker_size, y_unc=dMl_double2_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_double2', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                    plot_scatter_with_stats('Configuration', 'dMl', config_x_label, dMl_double2_data_current_gate, np.average(dMl_double2_data_current_gate), np.std(dMl_double2_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_double2', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                    plot_scatter_with_stats('Configuration', 'Mt', config_x_label, Mt_double2_data_current_gate, Mt_double2_combined_current_gate, Mt_double2_combined_sd_current_gate, Mt_double2_individual_unc_avg_current_gate, marker_size, y_unc=dMt_double2_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_double2', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                    plot_scatter_with_stats('Configuration', 'dMt', config_x_label, dMt_double2_data_current_gate, np.average(dMt_double2_data_current_gate), np.std(dMt_double2_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_double2', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                    plot_scatter_with_stats('Configuration', 'Fs', config_x_label, Fs_double2_data_current_gate, Fs_double2_combined_current_gate, Fs_double2_combined_sd_current_gate, Fs_double2_individual_unc_avg_current_gate, marker_size, y_unc=dFs_double2_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_double2', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                    plot_scatter_with_stats('Configuration', 'dFs', config_x_label, dFs_double2_data_current_gate, np.average(dFs_double2_data_current_gate), np.std(dFs_double2_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_double2', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                    plot_scatter_with_stats('Configuration', 'kp', config_x_label, kp_double2_data_current_gate, kp_double2_combined_current_gate, kp_double2_combined_sd_current_gate, kp_double2_individual_unc_avg_current_gate, marker_size, y_unc=dkp_double2_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_double2', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                    plot_scatter_with_stats('Configuration', 'dkp', config_x_label, dkp_double2_data_current_gate, np.average(dkp_double2_data_current_gate), np.std(dkp_double2_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_double2', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                    plot_scatter_with_stats('Configuration', 'keff', config_x_label, keff_double2_data_current_gate, keff_double2_combined_current_gate, keff_double2_combined_sd_current_gate, keff_double2_individual_unc_avg_current_gate, marker_size, y_unc=dkeff_double2_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_double2', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                    plot_scatter_with_stats('Configuration', 'dkeff', config_x_label, dkeff_double2_data_current_gate, np.average(dkeff_double2_data_current_gate), np.std(dkeff_double2_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_double2', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                    plot_scatter_with_stats('Configuration', 'Ml', config_x_label, Ml_double_both_data_current_gate, Ml_double_both_combined_current_gate, Ml_double_both_combined_sd_current_gate, Ml_double_both_individual_unc_avg_current_gate, marker_size, y_unc=dMl_double_both_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_double_both', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                    plot_scatter_with_stats('Configuration', 'dMl', config_x_label, dMl_double_both_data_current_gate, np.average(dMl_double_both_data_current_gate), np.std(dMl_double_both_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_double_both', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                    plot_scatter_with_stats('Configuration', 'Mt', config_x_label, Mt_double_both_data_current_gate, Mt_double_both_combined_current_gate, Mt_double_both_combined_sd_current_gate, Mt_double_both_individual_unc_avg_current_gate, marker_size, y_unc=dMt_double_both_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_double_both', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                    plot_scatter_with_stats('Configuration', 'dMt', config_x_label, dMt_double_both_data_current_gate, np.average(dMt_double_both_data_current_gate), np.std(dMt_double_both_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_double_both', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                    plot_scatter_with_stats('Configuration', 'Fs', config_x_label, Fs_double_both_data_current_gate, Fs_double_both_combined_current_gate, Fs_double_both_combined_sd_current_gate, Fs_double_both_individual_unc_avg_current_gate, marker_size, y_unc=dFs_double_both_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_double_both', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                    plot_scatter_with_stats('Configuration', 'dFs', config_x_label, dFs_double_both_data_current_gate, np.average(dFs_double_both_data_current_gate), np.std(dFs_double_both_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_double_both', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                    plot_scatter_with_stats('Configuration', 'kp', config_x_label, kp_double_both_data_current_gate, kp_double_both_combined_current_gate, kp_double_both_combined_sd_current_gate, kp_double_both_individual_unc_avg_current_gate, marker_size, y_unc=dkp_double_both_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_double_both', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                    plot_scatter_with_stats('Configuration', 'dkp', config_x_label, dkp_double_both_data_current_gate, np.average(dkp_double_both_data_current_gate), np.std(dkp_double_both_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_double_both', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                    plot_scatter_with_stats('Configuration', 'keff', config_x_label, keff_double_both_data_current_gate, keff_double_both_combined_current_gate, keff_double_both_combined_sd_current_gate, keff_double_both_individual_unc_avg_current_gate, marker_size, y_unc=dkeff_double_both_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_double_both', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                    plot_scatter_with_stats('Configuration', 'dkeff', config_x_label, dkeff_double_both_data_current_gate, np.average(dkeff_double_both_data_current_gate), np.std(dkeff_double_both_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_double_both', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                if use_user_specified_lifetime == True:
                                    plot_scatter_with_stats('Configuration', 'Ml', config_x_label, Ml_user_lifetime_data_current_gate, Ml_user_lifetime_combined_current_gate, Ml_user_lifetime_combined_sd_current_gate, Ml_user_lifetime_individual_unc_avg_current_gate, marker_size, y_unc=dMl_user_lifetime_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_user_lifetime', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                    plot_scatter_with_stats('Configuration', 'dMl', config_x_label, dMl_user_lifetime_data_current_gate, np.average(dMl_user_lifetime_data_current_gate), np.std(dMl_user_lifetime_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_user_lifetime', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                    plot_scatter_with_stats('Configuration', 'Mt', config_x_label, Mt_user_lifetime_data_current_gate, Mt_user_lifetime_combined_current_gate, Mt_user_lifetime_combined_sd_current_gate, Mt_user_lifetime_individual_unc_avg_current_gate, marker_size, y_unc=dMt_user_lifetime_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_user_lifetime', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                    plot_scatter_with_stats('Configuration', 'dMt', config_x_label, dMt_user_lifetime_data_current_gate, np.average(dMt_user_lifetime_data_current_gate), np.std(dMt_user_lifetime_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_user_lifetime', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                    plot_scatter_with_stats('Configuration', 'Fs', config_x_label, Fs_user_lifetime_data_current_gate, Fs_user_lifetime_combined_current_gate, Fs_user_lifetime_combined_sd_current_gate, Fs_user_lifetime_individual_unc_avg_current_gate, marker_size, y_unc=dFs_user_lifetime_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_user_lifetime', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                    plot_scatter_with_stats('Configuration', 'dFs', config_x_label, dFs_user_lifetime_data_current_gate, np.average(dFs_user_lifetime_data_current_gate), np.std(dFs_user_lifetime_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_user_lifetime', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                    plot_scatter_with_stats('Configuration', 'kp', config_x_label, kp_user_lifetime_data_current_gate, kp_user_lifetime_combined_current_gate, kp_user_lifetime_combined_sd_current_gate, kp_user_lifetime_individual_unc_avg_current_gate, marker_size, y_unc=dkp_user_lifetime_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_user_lifetime', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                    plot_scatter_with_stats('Configuration', 'dkp', config_x_label, dkp_user_lifetime_data_current_gate, np.average(dkp_user_lifetime_data_current_gate), np.std(dkp_user_lifetime_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_user_lifetime', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
                                    plot_scatter_with_stats('Configuration', 'keff', config_x_label, keff_user_lifetime_data_current_gate, keff_user_lifetime_combined_current_gate, keff_user_lifetime_combined_sd_current_gate, keff_user_lifetime_individual_unc_avg_current_gate, marker_size, y_unc=dkeff_user_lifetime_data_current_gate, show=False, save=current_save_path+str(current_gatewidth)+'_user_lifetime', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=True)
                                    plot_scatter_with_stats('Configuration', 'dkeff', config_x_label, dkeff_user_lifetime_data_current_gate, np.average(dkeff_user_lifetime_data_current_gate), np.std(dkeff_user_lifetime_data_current_gate, ddof=1), 0, marker_size, y_unc=None, show=False, save=current_save_path+str(current_gatewidth)+'_user_lifetime', yaxis_log=yaxis_log, show_avg=True, show_sd=True, show_unc_avg=False)
